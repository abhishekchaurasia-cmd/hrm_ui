...
import type { BusinessUnitEntity } from '@/features/admin/organization/api/organization';
...
const emptyForm: FormState = { name: '', legalEntityId: '', description: '' };

export function BusinessUnitsTab() {
  const { data: items = [], isLoading } = useBusinessUnits();
  ...
  const [editItem, setEditItem] = useState<BusinessUnitEntity | null>(null);
  const [form, setForm] = useState<FormState>(emptyForm);
  const [deleteTarget, setDeleteTarget] = useState<BusinessUnitEntity | null>(
    null
  );
  ...
  const openEdit = (item: BusinessUnitEntity) => {
    setEditItem(item);
    setForm({
      name: item.name,
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-muted-foreground text-sm">
          Manage business units for your organization
        </p>
      ...
      {isLoading ? (
        ...
        <p className="text-muted-foreground py-8 text-center text-sm">
          No business units yet. Click &quot;Add Business Unit&quot; to create
          one.
        </p>
      ) : (
        <Table>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {editItem ? 'Edit Business Unit' : 'Add Business Unit'}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={val =>
                  setForm(prev => ({ ...prev, legalEntityId: val }))
                }
              >
                <SelectTrigger>
